...
def index(m3u8):
    ...
    source = m3u8
    source = source.replace('https://urchin-app-kbhg3.ondigitalocean.app/', '')
...
def getm3u8():
    source = request.url
    source = source.replace('https://urchin-app-kbhg3.ondigitalocean.app/getm3u8?source=', '')
    source = source.replace('%2F', '/')
    source = source.replace('%3F', '?')
    headers = {
        "accept": "*/*",
...
def getstream():
    ...
    if param == "getts":
        source = request.url
        source = source.replace('https://urchin-app-kbhg3.ondigitalocean.app/getstream?param=getts&source=','')
        source = source.replace('%2F','/')
        source = source.replace('%3F','?')
        headers = {
            'accept': '*/*',
    ...
    if param == "getm3u8":
        ...
        r = requests.post("https://1xlite-1376721.top/cinema",json=veriler)
        if "FullscreenAllowed" in r.text:
            ...
            veri = veri.replace('edge100','edge10')
            if "m3u8" in veri:
                '''return "https://urchin-app-kbhg3.ondigitalocean.app/getm3u8?source="+veri+'&videoid='+videoid'''
                return "https://urchin-app-kbhg3.ondigitalocean.app/"+veri+'&videoid='+videoid
        else:
            return "Veri yok"
